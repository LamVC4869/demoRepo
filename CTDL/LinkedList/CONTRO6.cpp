...
bool solve (const LinkedList& myList) {
    ...
    node curr = prev->next;
    node next = curr->next;
    vector<int> listIndexValid;
    int index = 2;
    while (curr->next != nullptr) {
        if (isValid(prev->data, curr->data, next->data)) {
            listIndexValid.push_back(index);
        }
        ++index;
        prev = prev->next;
        curr = curr->next;
    ...
    if (listIndexValid.empty()) return false;
    cout << listIndexValid.size() << '\n';
    for (const auto& index : listIndexValid) {
        cout << index << ' ';
    }
    cout << '\n';
    return true;
